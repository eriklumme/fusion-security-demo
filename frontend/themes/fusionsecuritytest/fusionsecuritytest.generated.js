...
window.Vaadin['_vaadintheme_fusionsecuritytest_globalCss'] = window.Vaadin['_vaadintheme_fusionsecuritytest_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_fusionsecuritytest_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_fusionsecuritytest_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_fusionsecuritytest_globalCss'].push(target);
  }
  if (!document['_vaadintheme_fusionsecuritytest_componentCss']) {
    
    document['_vaadintheme_fusionsecuritytest_componentCss'] = true;
  }
